...
from robohermes_libero.catalog import suite_for, validate_short_catalog
...
def default_manifest_path() -> Path:
    ...
    if source.is_file():
        return source
    resource = importlib.resources.files("robohermes_libero").joinpath(
        "evidence/adaptive-pass10-v1/manifest.json"
    )
...
def replay_bundle(manifest_path: Path | str) -> ReplayResult:
    ...
    if manifest.get("schema") != "robohermes.libero_short_evidence.v1":
    ...
    result = ReplayResult(
        schema="robohermes.libero_short_replay.v1",
