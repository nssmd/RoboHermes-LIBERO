...
from robohermes_libero.catalog import SHORT_TASK_CATALOG
from robohermes_libero.config import ReleaseConfig, load_config
...
def _all_records(campaign_root: Path):
    from robohermes.embodied.sim.libero.run_records import load_records
...
def _runtime_env(
    ...
    *,
    ...
    env.update(
        {
            "ROBOHERMES_LIBERO_ROLES": "1",
            "ROBOHERMES_ATOMIC_COMPOUND": "1",
            "ROBOHERMES_RELEASE_ID": release_id,
            "ROBOHERMES_PROPOSAL_DIR": str(campaign_root / "proposals"),
            "ROBOHERMES_WORKSPACE": str(workspace_root or campaign_root / "workspace"),
            "ROBOHERMES_DATA_ROOT": str(campaign_root / "trajectories"),
            "ROBOHERMES_SELFEVO_FREEZE": "1" if manifest["mode"] == "fixed" else "0",
        }
    )
    return env
...
def run_assigned_tasks(
    ...
    *,
    ...
    if run_episode is None:
        from robohermes.embodied.skills.atomic._lib._libero_run import run_libero_episode
    ...
    for task in task_keys:
        if task in solved or ((task, seed) in terminal_pairs and not allow_changed_path):
            continue
        os.environ["ROBOHERMES_TASK_KEY"] = task
