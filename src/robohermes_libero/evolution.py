...
def validate_proposal(proposal: dict[str, Any]) -> ProposalValidation:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.ImportFrom):
            ...
            if module.startswith("robohermes.embodied.sim"):
                findings.append(f"forbidden direct simulator import: {module}")
...
def _default_harness(
    *,
    ...
    from robohermes.embodied.agent_loop.prompt_tools import _COMPOUND_CACHE, _PLUGIN_CACHE
    from robohermes.embodied.sim.libero.run_records import load_records
    from robohermes_libero.config import load_config
    from robohermes_libero.worker import run_assigned_tasks
    ...
    previous_workspace = os.environ.get("ROBOHERMES_WORKSPACE")
    _PLUGIN_CACHE.clear()
    ...
    try:
        ...
        if previous_workspace is None:
            os.environ.pop("ROBOHERMES_WORKSPACE", None)
        else:
            os.environ["ROBOHERMES_WORKSPACE"] = previous_workspace
        _PLUGIN_CACHE.clear()
        _COMPOUND_CACHE.clear()
    candidates = [
        row
    ...
    *,
    ...
    for path in sorted((root / "proposals").glob("*.json")):
        ...
        if not validation.ok:
            ...
            continue
        os.environ["ROBOHERMES_WORKSPACE"] = str(root / "workspace")
        try:
