...
from robohermes_libero.launcher import build_worker_commands
from robohermes_libero.protocol import CampaignState, EpisodeVerdict, schedule_round
...
def _source_records(campaign_root: Path):
    from robohermes.embodied.sim.libero.run_records import load_records, merge_records
...
def summarize_campaign(campaign_root: Path | str) -> dict[str, Any]:
    ...
    summary = {
        "schema": "robohermes.libero_short_campaign_summary.v1",
...
def run_supervisor(campaign_root: Path | str, *, max_stalled_retries: int = 6) -> dict[str, Any]:
    ...
    for seed in manifest["seeds"]:
        ...
        if manifest["mode"] == "adaptive":
            try:
                from robohermes_libero.evolution import process_pending_proposals
