...
def _client(host: str, port: int):
    ...
    # Lightweight client — only needs pyzmq + msgpack. The robohermes-sim
...
def predict_grasps_with_mask(
    ...
    """Maestro-aligned: build the GraspGen input cloud from a 2D OBJECT MASK
    (e.g. from Grounded-SAM) so only on-object pixels are unprojected. This
    is the correct way per Maestro paper Appendix A — feeding GraspGen a
