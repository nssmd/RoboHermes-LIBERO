...
from robohermes.embodied.agent_loop.env import (
    Backend,
    BackendUnavailable,
    Env,
    Observation,
...
# enumerated by ``list_tasks`` by default. Override via ROBOHERMES_LIBERO_SUITES
...
class LiberoProEnv(Env):
    ...
    def _preview_frame_budget() -> int:
        raw = os.environ.get("ROBOHERMES_LIBERO_PREVIEW_MAX_FRAMES", "256")
        try:
    ...
    def capture_orbit_views(self, *, image_size: int = 512):
        ...
        from robohermes.embodied.sim.libero.orbit_geometry import OrbitFrame
...
class LiberoProBackend(Backend):
    ...
    def __init__(self, suites: tuple[str, ...] = _DEFAULT_SUITES) -> None:
        env_suites = os.environ.get("ROBOHERMES_LIBERO_SUITES")
    ...
    def make_env(self, task: str, config: dict[str, Any] | None = None) -> Env:
        ...
        env = OffScreenRenderEnv(
            ...
            horizon=int(os.environ.get("ROBOHERMES_LIBERO_HORIZON", "5000")),  # Match the OpenETA standard-130 protocol; 4000 exhausted before recovery in 267/444 formal Pass@5 failures.
        )
        env.reset()
        return LiberoProEnv(
            env=env,
