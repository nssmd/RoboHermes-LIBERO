...
from robohermes.embodied.agent_loop.config import DEFAULT_MODEL, _skill_namespace
from robohermes.embodied.agent_loop.env import Observation, Rollout, Step
from robohermes.embodied.agent_loop.messages import (
    _append_image,
    _assistant_tool_calls_msg,
    _initial_messages,
    _summarize_old_trace,
)
from robohermes.embodied.agent_loop.prompt_tools import (
...
from robohermes.embodied.sim.libero.run_records import EpisodeIdentity
...
def _trajectory_capture_config() -> tuple[int, int]:
    ...
    return (
        positive_int("ROBOHERMES_TRAJECTORY_SUBSAMPLE", 20),
        positive_int("ROBOHERMES_TRAJECTORY_MAX_CAPTURES", 400),
    )
    ...
    return [
        {
            "role": "system",
            "content": (
                "You are the independent RoboHermes Recovery Reviewer. "
...
def _auto_complete_strong_place_enabled() -> bool:
    return os.environ.get(
        "ROBOHERMES_AUTO_COMPLETE_STRONG_PLACE",
        "0",
    ) == "1"
...
def run_rollout(
    ...
    *,
    ...
    reset_usage_metrics()
    if workdir is None:
        workdir = Path("/tmp/robohermes-zeroshot") / f"{task_name}-{seed}"
    else:
    ...
    if (
        os.environ.get("ROBOHERMES_ATOMIC_COMPOUND") == "1"
        and os.environ.get("ROBOHERMES_SELFEVO_FREEZE", "0") == "0"
    ):
        from robohermes.embodied.skills import discover_compounds
...
def _persist_episode(
    ...
    *,
    ...
    exact failure the predicate exists to catch. Set ROBOHERMES_COLLECT=0 to skip.
    """
    import os
    if os.environ.get("ROBOHERMES_COLLECT", "1") == "0" or not success:
        return None
    if not rollout.steps:
        return None
    from robohermes.data.store import DataStore
...
def _dispatch(
    ...
    *,
    ...
    if handler is None and state.task and \
            os.environ.get("ROBOHERMES_ATOMIC_COMPOUND") == "1" and \
            os.environ.get("ROBOHERMES_SELFEVO_FREEZE", "0") == "0":
