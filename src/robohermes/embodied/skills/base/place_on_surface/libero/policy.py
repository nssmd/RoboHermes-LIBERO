...
_LOCAL_SUPPORT_MIN_DROP = 0.04
_LOCAL_SUPPORT_MAX_DROP = 0.20
_FLAT_SUPPORT_WORDS = frozenset({"plate", "stove", "pad", "scale"})
...
_SURFACE_TARGET_CACHE_ATTR = "_robohermes_surface_target_cache"
...
def _local_support_height_below_occluder(
    env: Any,
    *,
    pixel: tuple[int, int],
    ...
    if not words.intersection(_FLAT_SUPPORT_WORDS):
        return None
    width, height = image_size
    scale = max(0.5, min(width, height) / 512.0)
    radii = (max(2, int(round(24 * scale))), max(3, int(round(40 * scale))))
    directions = (
        (-1, 0),
        (1, 0),
        (0, -1),
    ...
    for radius in radii:
        for dx, dy in directions:
            u = int(pixel[0] + dx * radius)
            v = int(pixel[1] + dy * radius)
            if not (0 <= u < width and 0 <= v < height):
                continue
            try:
                point = _valid_world_point(env.pixel_to_world(u, v))
    ...
    if len(heights) < 4:
        return None
    candidate = float(np.percentile(np.asarray(heights, dtype=float), 25))
    drop = float(reference_height) - candidate
    if not _LOCAL_SUPPORT_MIN_DROP <= drop <= _LOCAL_SUPPORT_MAX_DROP:
        return None
    return candidate
...
def _resolve_surface_target(
    ...
    *,
    ...
    from robohermes.embodied.skills.base._lib.libero._perception import object_cloud
    ...
    target_source = "sam_cloud"
    local_support_height = _local_support_height_below_occluder(
        env,
        pixel=uv,
        image_size=image_size,
        target_name=target_name,
        reference_height=float(cloud_point[2]),
    )
    if local_support_height is not None:
        cloud_point[2] = local_support_height
        target_source = "sam_cloud_local_support_below_occluder"
    return _cache_surface_target(
        env,
        cache_key,
        SurfaceTarget(uv, cloud_point, target_source),
    )
