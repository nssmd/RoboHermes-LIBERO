...
from robohermes.embodied.skills.base._lib.libero.semantic_point import (
    matching_semantic_point,
)
from robohermes.embodied.skills.base.pull_drawer.libero.policy import (
...
def dispatch_runtime(state: Any, args: dict[str, Any]):
    ...
    words = _semantic_words(object_name)
    ...
    orientation_candidates = _side_entry_quat_candidates(normal)
    quat = np.asarray(orientation_candidates[0], dtype=float)
    orientation_candidate_index = None
    orientation_mode = "direct_side_entry"
    ...
    direct_reached = bool(
        reached and _measured_close(approach, measured, 0.05)
    )
    orientation_attempts.append(
        {
            "index": 0,
            "direct_approach_reached": direct_reached,
            "staging_orientation_reached": None,
            "staged_approach_reached": None,
            "restaged": None,
        }
    )
    if direct_reached:
        orientation_candidate_index = 0
    else:
        posture_recovered, _ = ctrl.recover_ready_posture(max_iters=180)
        current_pos, current_quat, _ = ctrl.read_pose()
        ...
        measured, _, _ = ctrl.read_pose()
        stage_reached = bool(
            stage_reached and _measured_close(stage, measured, 0.06)
        )
        if stage_reached:
            preserve_attempt = {
                "index": -1,
                "direct_approach_reached": None,
                "staging_orientation_reached": True,
                "staged_approach_reached": False,
                "restaged": None,
            }
            orientation_attempts.append(preserve_attempt)
            reached, _ = ctrl.servo_to(
                approach,
                quat=np.asarray(current_quat, dtype=float),
                gripper="open",
            ...
            measured, _, _ = ctrl.read_pose()
            preserve_attempt["staged_approach_reached"] = bool(
                reached and _measured_close(approach, measured, 0.05)
            )
            if preserve_attempt["staged_approach_reached"]:
                quat = np.asarray(current_quat, dtype=float)
                orientation_candidate_index = -1
                orientation_mode = "staged_preserve"
            else:
                restaged, _ = ctrl.servo_to(
                    stage,
                    quat=np.asarray(current_quat, dtype=float),
                    gripper="open",
                    pos_tol=0.05,
                ...
                measured, _, _ = ctrl.read_pose()
                preserve_attempt["restaged"] = bool(
                    restaged and _measured_close(stage, measured, 0.06)
                )
                stage_reached = bool(preserve_attempt["restaged"])
        if stage_reached and orientation_candidate_index is None:
            for index, candidate_quat in enumerate(orientation_candidates):
                attempt = (
                ...
                orientation_reached, _ = ctrl.servo_to(
                    stage,
                    quat=candidate_quat,
                    gripper="open",
                    pos_tol=0.05,
                ...
                measured, _, _ = ctrl.read_pose()
                attempt["staging_orientation_reached"] = bool(
                    orientation_reached
                    and _measured_close(stage, measured, 0.06)
                )
                if not attempt["staging_orientation_reached"]:
                    continue
                reached, _ = ctrl.servo_to(
                    approach,
                    quat=candidate_quat,
                ...
                measured, _, _ = ctrl.read_pose()
                attempt["staged_approach_reached"] = bool(
                    reached and _measured_close(approach, measured, 0.05)
                )
                if attempt["staged_approach_reached"]:
                    quat = np.asarray(candidate_quat, dtype=float)
                ...
                if not attempt["restaged"]:
                    break
        if orientation_candidate_index is None:
            return _failure(
                env,
                "could not reach the door-handle approach pose",
                posture_recovered=posture_recovered,
                staging_reached=stage_reached,
                orientation_candidate_index=None,
    ...
    nominal_sign = 1.0 if float(np.dot(tangent_ccw, normal)) >= 0.0 else -1.0
    arc_steps = max(5, int(math.ceil(requested_angle / _ARC_STEP_DEG)))
    achieved_angle = 0.0
    final_normal = normal.copy()
    angle_values = np.linspace(0.0, requested_angle, arc_steps + 1)
    arc_direction_sign = None
    arc_direction_attempts = []

    def _arc_target(angle_deg: float, direction: float):
        angle = math.radians(float(angle_deg)) * float(direction)
        rotated_radial = _rotate_xy(radial, angle)
        rotated_normal = _rotate_xy(normal, angle)
        target_handle = np.asarray(hinge, dtype=float) + radius * rotated_radial
        target_handle[2] = float(handle[2])
        return target_handle - rotated_normal * _CONTACT_OFFSET, rotated_normal

    first_angle = float(angle_values[1])
    for direction in (nominal_sign, -nominal_sign):
        target, candidate_normal = _arc_target(first_angle, direction)
        arc_quat = (
            quat
            if orientation_candidate_index == -1
            else _side_entry_quat_candidates(candidate_normal)[
                orientation_candidate_index
            ]
        )
        reached, _ = ctrl.servo_to(
            target,
        ...
        first_reached = bool(
            reached
            and _measured_close(target, measured, 0.04)
            and hold_state is GripperState.HELD
        )
        attempt = {
        ...
        arc_direction_attempts.append(attempt)
        if first_reached:
            arc_direction_sign = float(direction)
            final_normal = candidate_normal
            achieved_angle = first_angle
            break
        reset_reached, _ = ctrl.servo_to(
            contact,
            quat=quat,
            gripper="close",
            pos_tol=0.025,
        ...
        measured, _, _ = ctrl.read_pose()
        _, reset_hold = ctrl.read_gripper_state()
        attempt["reset_reached"] = bool(
            reset_reached
            and _measured_close(contact, measured, 0.04)
            and reset_hold is GripperState.HELD
        )
        attempt["reset_holding"] = bool(
            reset_hold is GripperState.HELD
        )
        if not attempt["reset_holding"]:
            break

    if arc_direction_sign is not None:
        for angle_deg in angle_values[2:]:
            target, final_normal = _arc_target(
                float(angle_deg), arc_direction_sign
            )
            arc_quat = (
                quat
                if orientation_candidate_index == -1
                else _side_entry_quat_candidates(final_normal)[
                    orientation_candidate_index
                ]
            )
            reached, _ = ctrl.servo_to(
    ...
    return (
        {
            ...
            "orientation_attempts": orientation_attempts,
            "nominal_arc_direction_sign": float(nominal_sign),
            "arc_direction_sign": arc_direction_sign,
            "arc_direction_attempts": arc_direction_attempts,
        },
        env.take_snapshot(),
    )
