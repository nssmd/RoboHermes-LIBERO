...
Reuses two embodiment-neutral pieces from the LIBERO stack:
...
_HEAD = "agentview"
_FRONT_RELATION_FIXTURES = frozenset(
    {"cabinet", "drawer", "microwave", "rack", "stove"}
)
_MOVABLE_OBJECT_WORDS = frozenset(
    {"bottle", "bowl", "box", "can", "carton", "mug", "package", "plate"}
)
...
def _fix_on() -> bool:
    ...
    ``ROBOHERMES_GRASP_FIX=0`` to fall back to the old unguarded path."""
    return os.environ.get("ROBOHERMES_GRASP_FIX", "1") != "0"
...
def vlm_point(state, obj: str, location: str = ""):
    ...
    import os
    import cv2
    from robohermes.embodied.agent_loop.config import _POINT_SYSTEM_PROMPT
    from robohermes.embodied.agent_loop.vlm_io import _call_vlm_image, _parse_json
    ...
    scale = int(os.environ.get("ROBOHERMES_POINT_UPSCALE", "3"))
    if scale > 1:
    ...
    model = os.environ.get("ROBOHERMES_PERCEPTION_MODEL", "responses/gpt-5.6-sol")
...
def _is_fixture_front_support_relation(value: str) -> bool:
    words = _query_words(value)
    return bool(
        "front" in words
        and words.intersection(_FRONT_RELATION_FIXTURES)
        and not words.intersection(_MOVABLE_OBJECT_WORDS)
    )
...
def _semantic_point_query(obj: str) -> str:
    ...
    if (
        _is_fixture_front_support_relation(text)
    ):
        fixture = sorted(words & _FRONT_RELATION_FIXTURES)[0]
        return (
            f"{text}; this names a destination relation, not the fixture: "
            "identify the fixture, then point to the center of the free "
            "horizontal supporting surface immediately outside its visible "
            "front edge, close enough for the manipulated object to rest "
            "there; point to the final destination, not halfway from the "
            f"source, and not on the {fixture}, "
            "burner, fixture body, handle, or control"
        )
    if "plate" in words:
        return (
            f"{text}; choose the visible flat round dinner plate resting on "
            "the horizontal support, not a stove burner, bowl, pan, robot, "
            "gripper, or another circular object; if partly occluded, point "
            "to the visible plate body rather than the occluder"
        )
    if {"cookie", "box"} <= words:
        return (
...
def _refine_semantic_point(state, obj: str, uv):
    if _is_fixture_front_support_relation(obj):
...
def local_vlm_point(state, obj: str):
    ...
    instead of loading the GPT tunnel. (Molmo was evaluated too but its image
    ...
    port = int(os.environ.get("ROBOHERMES_VLM_PORT", "0"))
    if not port:
        return None
    rgb = state.env.take_snapshot().images.get("head_camera")
    if rgb is None:
...
def _ranked_localization_candidates(rgb, obj: str, pointer_uv=None):
    ...
    try:
        from robohermes.embodied.skills.base._lib.libero.detector import (
            detect,
        )
...
def _choose_localization_candidate(
    ...
    import cv2
    from robohermes.embodied.agent_loop.vlm_io import (
        _call_vlm_image,
        _parse_json,
    )
    ...
    scale = int(
        os.environ.get("ROBOHERMES_CANDIDATE_UPSCALE", "3")
    )
    if scale > 1:
        image = cv2.resize(
            image,
    ...
    workdir = Path(
        getattr(
            state,
            "workdir",
            "/tmp/robohermes-localization-candidates",
        )
    )
    ...
    min_confidence = float(
        os.environ.get(
            "ROBOHERMES_CANDIDATE_MIN_CONFIDENCE",
            "0.7",
        )
    )
...
def _sam_refine_point(state, uv):
    ...
    max_shift = float(
        os.environ.get("ROBOHERMES_SAM_REFINE_MAX_SHIFT_PX", "24")
    )
    shift = float(
        np.linalg.norm(
...
def locate_pixel(rgb, query: str, scale: int = 3):
    ...
    import cv2
    from robohermes.embodied.skills.base._lib.libero.detector import detect
    rgb = np.asarray(rgb)
...
def locate_by_sam3(rgb, target: str, threshold: float = 0.1):
    ...
    Gated on ROBOHERMES_SAM3_PORT (unset → skip, graceful fallback)."""
    import os
    import pickle
    port = int(os.environ.get("ROBOHERMES_SAM3_PORT", "0"))
    if not port:
        return None
    import zmq
    sock = zmq.Context.instance().socket(zmq.REQ)
...
def _place_fix_on() -> bool:
    ...
    read 'arm occludes the plate'. Set ``ROBOHERMES_PLACE_CLEAR=0`` to disable."""
    return os.environ.get("ROBOHERMES_PLACE_CLEAR", "1") != "0"
...
def localize_precise(state, obj: str, route: str = "vlm_sam"):
    ...
    Engineer picks (via the find_by_detector / find_by_pointing skills):
      ...
      * 'vlm_sam' — POINT→MASK: a VLM pointer (local Qwen2.5-VL, else GPT
    ...
    from robohermes.embodied.sim.libero.run_records import (
    ...
    if (
        ...
        and os.environ.get(
            "ROBOHERMES_GPT_POINTER_AUTHORITATIVE",
            "0",
        )
        == "1"
    ):
        return _refine_semantic_point(state, obj, remote_uv)
    uv = None
    consensus_uv = None
    ...
    if consensus_uv is not None and not depth_relation_query:
        return _refine_semantic_point(state, obj, consensus_uv)
    ...
    if relation_selected is not None:
        return _refine_semantic_point(state, obj, relation_selected)
    selected = None
    if depth_relation_query and candidates:
        ...
        if selected is not None:
            return _refine_semantic_point(state, obj, selected)
    ...
    if uv is not None:
        return _refine_semantic_point(state, obj, uv)
...
def sam_mask_at_point(rgb, u: int, v: int) -> np.ndarray:
    ...
    import torch
    from PIL import Image
    from robohermes.embodied.skills.base._lib.libero.detector import _load
    _, _, sp, sm = _load()
...
def geometric_grasp_candidates(cloud) -> list[dict[str, Any]]:
    """Pure point-cloud top-down fallback when the optional service is absent."""
    ...
    center[2] = float(np.median(points[:, 2]))
    return [
        {
            "score": 0.0,
            "translation_world": center.tolist(),
            "translation_tcp_world": center.tolist(),
            "rotation_matrix_world": None,
            "num_object_points": int(len(points)),
            "source": "geometric_topdown_fallback",
        }
    ]
...
def grasps_at_pixel(env, u: int, v: int, top_k: int = 3):
    ...
    if cloud is None:
        return [], None
    port = int(os.environ.get("GRASPGEN_PORT", "0") or 0)
    if port <= 0:
        grasps = geometric_grasp_candidates(cloud)
    else:
        try:
            from robohermes.embodied.sim.libero.graspgen_client import _grasps_from_cloud
            ...
            print(
                f"[grasp] optional GraspGen unavailable ({type(exc).__name__}); "
                "using geometric top-down fallback",
                flush=True,
            )
            grasps = geometric_grasp_candidates(cloud)
    return filter_grasps_consistent_with_cloud(grasps, cloud), cloud
...
def filter_grasps_consistent_with_cloud(
    ...
    *,
    ...
    if max_distance is None:
        max_distance = float(
            os.environ.get(
                "ROBOHERMES_GRASP_CLOUD_MAX_DISTANCE",
                "0.06",
            )
        )
...
def execute_rim_grip(
    ...
    *,
    ...
    from robohermes.embodied.skills.base._lib.libero._control import LiberoControl
    from robohermes.embodied.skills.base._lib.libero.gripper_state import (
        GripperState,
    )
...
def _6dof_on() -> bool:
    ...
    return os.environ.get("ROBOHERMES_GRASP_6DOF", "0") != "0"
...
def execute_package_side_entry(
    ...
    *,
    ...
    from robohermes.embodied.skills.base._lib.libero._control import (
        LiberoControl,
    )
    from robohermes.embodied.skills.base._lib.libero.gripper_state import (
        GripperState,
...
def execute_topdown(
    ...
    """Grasp at GraspGen's chosen point. With ``ROBOHERMES_GRASP_6DOF`` on (default)
...
def execute_base_grip(
    ...
    from robohermes.embodied.skills.base._lib.libero._control import LiberoControl
    from robohermes.embodied.skills.base._lib.libero.gripper_state import GripperState
    ctrl = LiberoControl(env)
