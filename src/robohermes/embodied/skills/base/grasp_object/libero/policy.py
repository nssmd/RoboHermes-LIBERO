...
from robohermes.embodied.skills.base._lib.libero.visual_hold import (
    capture_depth_frame,
    clear_visual_hold,
...
def _fix_on() -> bool:
    ...
    right-object. ``ROBOHERMES_GRASP_FIX=0`` restores the old unguarded path."""
...
def _verify_object_pixel(
    ...
    workdir = Path(
        getattr(state, "workdir", "/tmp/robohermes-grasp-identity")
    )
...
def _locate_pixel(state, args):
    ...
    pix = args.get("pixel")
    if pix is not None:
        from robohermes.embodied.skills.base._lib.libero._helpers import (
            parse_image_pixel,
        )
        ...
        return None if _is_sentinel(uv) else uv
    if name:
        from robohermes.embodied.skills.base._lib.libero._perception import (
...
def _perception_grasp(state, args):
    ...
    if _is_sentinel(loc):
        ...
        return ({"ok": False, "grasped": False, "sentinel": True,
                ...
                env.take_snapshot())
    u, v = loc
    explicit_pixel = args.get("pixel") is not None
    ...
    if not grasps and _fix_on() and rim_plan is None:
        ...
        if name:
            ...
            if loc2 and not _is_sentinel(loc2):
                ...
                if (
                    explicit_pixel
                    and not _semantic_relocalization_consistent(
                        (u, v),
                        candidate,
                    )
                ):
                    candidate = None
                elif _requires_semantic_pointing(name):
    ...
    package_side_entry_attempted = False
    ...
    if (
        ...
        package_side_entry_attempted = True
        ...
        package_side_entry_succeeded = bool(
            side_evidence.get("side_entry_succeeded")
        )
        package_side_candidates_attempted = int(
            side_evidence.get("side_candidates_attempted") or 0
        )
        package_side_candidate_index = side_evidence.get(
            "side_candidate_index"
        )
        gap_raw, grip_state = ctrl.read_gripper_state()
        gap = round(float(gap_raw), 4)
        grasped = bool(
            package_side_entry_succeeded
    ...
    return ({"ok": True, "grasped": grasped, "backend": "graspgen+sam",
             ...
             "package_side_entry_attempted": package_side_entry_attempted,
             "package_side_entry_succeeded": package_side_entry_succeeded,
             "package_side_candidates_attempted": (
                 package_side_candidates_attempted
             ),
             "package_side_candidate_index": package_side_candidate_index,
