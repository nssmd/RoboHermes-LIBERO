...
from robohermes.embodied.skills.base._lib.libero.drawer_evidence import (
...
_LAYER_DIRECTION_TOL = 12
_MIDDLE_LAYER_MAX_SHIFT = 36
...
def _side_entry_quat_candidates(normal: np.ndarray) -> tuple[np.ndarray, ...]:
    from scipy.spatial.transform import Rotation

    base = Rotation.from_quat(_side_entry_quat(normal))
    symmetric = base * Rotation.from_rotvec(
        np.array([0.0, 0.0, np.pi], dtype=float)
    )
    return (
        np.asarray(base.as_quat(), dtype=float),
        np.asarray(symmetric.as_quat(), dtype=float),
    )
...
def _verify_drawer_handle_pixel(
    ...
    from robohermes.embodied.skills.base._lib.libero._perception import (
        write_image_atomic,
    )
    ...
    scale = max(
        2,
        int(os.environ.get("ROBOHERMES_CANDIDATE_UPSCALE", "3")),
    )
    full = cv2.resize(
        image,
    ...
    workdir = Path(
        getattr(state, "workdir", "/tmp/robohermes-drawer-verify")
    )
    ...
    minimum_confidence = float(
        os.environ.get(
            "ROBOHERMES_DRAWER_VERIFY_MIN_CONFIDENCE",
            "0.65",
        )
    )
    return bool(
...
def _resolve_drawer_handle(
    ...
    def layer_consistent(candidate_uv: tuple[int, int]) -> bool:
        if layer == "top":
            return int(candidate_uv[1]) <= int(uv[1]) + _LAYER_DIRECTION_TOL
        if layer == "bottom":
            return int(candidate_uv[1]) >= int(uv[1]) - _LAYER_DIRECTION_TOL
        if layer == "middle":
            return (
                abs(int(candidate_uv[1]) - int(uv[1]))
                <= _MIDDLE_LAYER_MAX_SHIFT
            )
        return True
    ...
    if layer is not None:
        for candidate in _layer_handle_candidates(
            ...
            if not layer_consistent(candidate[0]):
                continue
            last_uv = candidate[0]
    ...
    if (
        refined is not None
        and layer_consistent(refined[0])
        and _verify_drawer_handle_pixel(
            state,
            object_name,
            rgb,
            refined[0],
    ...
    if corrected_uv is None:
        return None
    if not layer_consistent(corrected_uv):
        return None
...
def dispatch_runtime(state: Any, args: dict[str, Any]):
    ...
    if not stage_reached or not _measured_close(stage, measured, 0.06):
        return (
            {
                "ok": False,
                "pulled": False,
                "reason": "could not reach the high handle staging pose",
            },
            env.take_snapshot(),
        )
    ...
    for index, candidate_quat in enumerate(orientation_candidates):
        ...
        measured, measured_quat, _ = ctrl.read_pose()
        approach_reached = _measured_close(approach, measured, 0.05)
        attempt["approach_reached"] = bool(approach_reached)
        if approach_reached:
            active_quat = np.asarray(measured_quat, dtype=float)
            if active_quat.shape != (4,) or not np.all(np.isfinite(active_quat)):
                active_quat = np.asarray(candidate_quat, dtype=float)
    ...
    candidate_quat = np.asarray(measured_quat, dtype=float)
    if candidate_quat.shape == (4,) and np.all(np.isfinite(candidate_quat)):
        active_quat = candidate_quat
