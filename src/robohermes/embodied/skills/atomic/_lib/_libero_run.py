...
from robohermes.embodied.agent_loop import get_backend
from robohermes.embodied.agent_loop.rollout import run_rollout
...
from robohermes.embodied.skills import get as get_skill
...
def _code_fingerprint() -> str:
    release_id = os.environ.get("ROBOHERMES_RELEASE_ID", "").strip()
...
def _config_fingerprint(payload: dict[str, Any]) -> str:
    ...
    fields = (
        ...
        ("roles", os.environ.get("ROBOHERMES_LIBERO_ROLES", "0")),
        (
...
def _role_orchestrated_instruction(
    *,
    ...
    """Run the real RoboHermes Planner before the in-process Engineer loop."""
    from robohermes.agents import Planner
    from robohermes.agents.workspace import Workspace
    ...
    instruction = (
        ...
        f"{guidance}\n\n"
        f"PLAN (from RoboHermes Planner):\n{plan_md}\n\n"
...
def _review_role_episode(
    *,
    ...
    from robohermes.agents import Reviewer
...
def run_libero_atomic(
    ...
    *,
    ...
    work = Path(workdir).expanduser() if workdir else Path("/tmp/robohermes-libero")
...
def run_libero_episode(
    ...
    *,
    ...
    roles_enabled = os.environ.get("ROBOHERMES_LIBERO_ROLES", "0") == "1"
    role_workspace = None
    ...
    with be.make_env(task) as env:
        ...
        if roles_enabled:
            ...
            rollout_workdir = role_workspace.root / "maestro"
        try:
            res = run_rollout(
                env,
                seed=seed,
    ...
    meta["efficiency_schema"] = "robohermes.efficiency.v2"
