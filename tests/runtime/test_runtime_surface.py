...
def test_runtime_registers_only_libero_backend() -> None:
    from robohermes.embodied.agent_loop.registry import list_backends

    assert list_backends() == ["libero"]


def test_runtime_defaults_to_gpt_responses_medium() -> None:
    from robohermes.embodied.agent_loop.config import DEFAULT_MODEL
    from robohermes.embodied.agent_loop.vlm_io import _responses_reasoning_effort
...
def test_visible_tool_surface_excludes_hidden_simulator_truth(monkeypatch) -> None:
    from robohermes.embodied.agent_loop.prompt_tools import _build_tool_specs

    monkeypatch.setenv("ROBOHERMES_ATOMIC_COMPOUND", "1")
...
def test_exported_runtime_contains_no_non_gpt_provider_or_private_default() -> None:
    root = Path(__file__).resolve().parents[2] / "src/robohermes"
    forbidden = (
        "anthropic",
        "claude",
        "/mnt" + "/workspace",
