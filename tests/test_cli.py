...
from robohermes_libero.cli import app

runner = CliRunner()


def test_configure_creates_one_canonical_yaml(tmp_path: Path) -> None:
    config = tmp_path / "robohermes.yaml"
...
def test_results_replay_emits_machine_readable_json(tmp_path: Path) -> None:
    ...
    (bundle / "manifest.json").write_text(
        '{"schema":"robohermes.libero_short_evidence.v1",'
    ...
    (bundle / "episodes.jsonl").write_text(
        '{"schema":"robohermes.libero_episode.v1","task_key":"libero_goal/0",'
