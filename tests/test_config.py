...
def test_resolved_environment_contains_no_literal_secret(tmp_path: Path) -> None:
    ...
    assert env["ROBOHERMES_VLM_MODEL"] == "responses/gpt-5.6-sol"
    assert env["ROBOHERMES_REASONING_EFFORT"] == "medium"
    assert env["ROBOHERMES_OPENAI_API_KEY"] == "secret-value"
